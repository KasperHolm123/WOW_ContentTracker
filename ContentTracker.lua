...
function CTUI_StatusBar()
    ...
    x:SetScript("OnMouseDown", function ()
        x:GetParent():StartMoving()
    end)
    x:SetScript("OnMouseUp", function ()
        x:GetParent():StopMovingOrSizing()
    end)
    x:GetParent():SetScript("OnSizeChanged", function ()
        x:SetSize((x:GetParent():GetWidth() - 24), 30)
    end)
end
